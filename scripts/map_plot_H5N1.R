...
localTreesDirectory = "H5N1_extracted_trees"
#allTrees = scan(file="H5N1_host_lat_long.burnin.0.1.trees", what="", sep="\n", quiet=TRUE)
burnIn = 0
...
mcc_tre = readAnnotatedNexus("H5N1_host_lat_long.mean.mcc.tree")
mcc_tab = mccExtractions(mcc_tre, mostRecentSamplingDatum)
write.csv(mcc_tab, "H5N1_MCC.csv", row.names=F, quote=F)
...
#minYear = 2020.633; maxYear = 2022.5
minYear = 2020.633; maxYear = 2021.5
...
pdf(sprintf('map_H5N1_%s_v2.pdf',maxYear),width=16, height=15,bg="white")
...
rast = raster(matrix(nrow=1, ncol=2)); rast[1] = min(mcc_tab_this[,"startYear"]); rast[2] = 2021.6
