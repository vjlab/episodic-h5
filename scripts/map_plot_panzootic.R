...
localTreesDirectory = "panzootic_extracted_trees"
#allTrees = scan(file="panzootic_HA_host_lat_long.burnin.0.1.trees", what="", sep="\n", quiet=TRUE)
burnIn = 0
...
mcc_tre = readAnnotatedNexus("panzootic_HA_host_lat_long.mean.mcc.tree")
mcc_tab = mccExtractions(mcc_tre, mostRecentSamplingDatum)
write.csv(mcc_tab, "panzootic_MCC.csv", row.names=F, quote=F)
...
#minYear = 2017.168; maxYear = 2022.466
minYear = 2017.168; maxYear = 2020.5
...
pdf('map_panzootic_v2.pdf',width=16, height=15,bg="white")
...
rast = raster(matrix(nrow=1, ncol=2)); rast[1] = min(mcc_tab_this[,"startYear"]); rast[2] = 2020.5
